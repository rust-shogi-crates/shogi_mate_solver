...
This file is the issue-level control plan for the remaining NNUE move-ordering work. It is intended to be merged into `main` and used to spawn smaller PRs. Do not implement multiple work items from this file in one PR.
...
- Remove PR-local `codex-notes` right before merge, unless the note is intentionally issue-level guidance like this file.
...
The remaining work starts from the merged feature extractor. The next PR should not add neural-network inference yet; it should add the smallest scorer interface that can consume sparse `FeatureId` lists and affect ordering only in an explicit mode.

## Work Items

### Work Item 1: Baseline Learned-Score Interface
...
### Work Item 2: NNUE-Style Inference Runtime

- Status: pending Work Item 1.
...
### Work Item 3: Training and Export Pipeline

- Status: pending Work Item 2.
...
### Work Item 4: Trained Model Rollout

- Status: pending Work Item 3 and benchmark evidence.
