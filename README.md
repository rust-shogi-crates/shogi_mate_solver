...
`nnue_training` は benchmark の検索結果から小さな move-ordering 用の学習データを作り、ランタイム形式へ export する。学習例の label は候補手ごとの子局面を指定した evaluator で再評価し、攻め方にとって詰みが証明された手を `1`、それ以外を `0` とする。生成物は repository に commit しない。
